...
const EditTipeMotor = () => {
  ...
  const updateTipeMotor = async (e) => {
    e.preventDefault()

    try {
      await axios.patch(
        `http://localhost:5000/tipe-motor/${id}`,
        {
          name,
        },
        {
  ...
  const getTipeMotorById = async () => {
    const response = await axios.get(`http://localhost:5000/tipe-motor/${id}`)
  ...
  return (
    <>
      ...
      <CCard className="mb-4">
        <CCardHeader>
          <strong>Edit Tipe Motor</strong>
        </CCardHeader>
        <CCardBody>
          <CForm onSubmit={updateTipeMotor}>
            <div className="mb-3">
              <CFormInput
                type="text"
